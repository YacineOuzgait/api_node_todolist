...
class BaseService {
  ...
  static getTables = async () => {
    ...
    return await BaseService.executeQuery(sql);
  };

  static initialize = async () => {
    const tables = await BaseService.getTables();
    for (const table of tables) {
      const tableColumns = await BaseService.executeQuery(`SHOW FULL COLUMNS FROM ${table.name}`);
      const columns = {};
